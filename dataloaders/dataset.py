...
class VideoDataset(Dataset):
    ...
    def __init__(self, dataset='ucf101', split='train', clip_len=16, preprocess=False):
        self.root_dir, self.output_dir, self.bbox_output_dir = Path.db_dir(dataset)
    ...
    def process_video(self, video, action_name, save_dir):
        ...
        while (count < frame_count and retaining):
            ...
            if count % EXTRACT_FREQUENCY == 0:
                ...
                cv2.imwrite(filename=os.path.join(save_dir, video_filename, '%06d.jpg' % i), img=frame)
                i += 1
            count += 1
    ...
    # def load_bbox(self, file_dir):
    #     buffer = np.empty((, clip_len, 100, 4), np.dtype('float32'))
